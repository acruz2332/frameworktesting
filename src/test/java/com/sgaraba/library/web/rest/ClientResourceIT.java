...
class ClientResourceIT {
    ...
    @Test
    @Transactional
    void getAllClientsByBorrowedBookIsEqualToSomething() throws Exception {
        BorrowedBook borrowedBook;
        if (TestUtil.findAll(em, BorrowedBook.class).isEmpty()) {
            clientRepository.saveAndFlush(client);
            borrowedBook = BorrowedBookResourceIT.createEntity(em);
        } else {
            borrowedBook = TestUtil.findAll(em, BorrowedBook.class).get(0);
        }
        em.persist(borrowedBook);
        em.flush();
        client.setBorrowedBook(borrowedBook);
        borrowedBook.setClient(client);
        clientRepository.saveAndFlush(client);
        Long borrowedBookId = borrowedBook.getId();
        // Get all the clientList where borrowedBook equals to borrowedBookId
        defaultClientShouldBeFound("borrowedBookId.equals=" + borrowedBookId);

        // Get all the clientList where borrowedBook equals to (borrowedBookId + 1)
        defaultClientShouldNotBeFound("borrowedBookId.equals=" + (borrowedBookId + 1));
    }
    ...
    void partialUpdateClientWithPatch() throws Exception {
        ...
        partialUpdatedClient.firstName(UPDATED_FIRST_NAME).email(UPDATED_EMAIL).address(UPDATED_ADDRESS).phone(UPDATED_PHONE);
