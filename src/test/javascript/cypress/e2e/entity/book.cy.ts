...
describe('Book e2e test', () => {
  ...
  const bookSample = { isbn: 'glittering', name: 'polarisation what', publishYear: 'wholly vast challenge', copies: 6130 };
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="isbn"]`).type('knotty icy ha');
      cy.get(`[data-cy="isbn"]`).should('have.value', 'knotty icy ha');

      cy.get(`[data-cy="name"]`).type('overcook');
      cy.get(`[data-cy="name"]`).should('have.value', 'overcook');

      cy.get(`[data-cy="publishYear"]`).type('apud so');
      cy.get(`[data-cy="publishYear"]`).should('have.value', 'apud so');

      cy.get(`[data-cy="copies"]`).type('20438');
      cy.get(`[data-cy="copies"]`).should('have.value', '20438');
