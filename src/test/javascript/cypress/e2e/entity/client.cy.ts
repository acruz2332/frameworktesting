...
describe('Client e2e test', () => {
  ...
  const clientSample = { firstName: 'Adan', lastName: 'Friesen' };
  ...
  describe('Client page', () => {
    ...
    describe('with existing value', () => {
      ...
      it.skip('edit button click should load edit Client page and save', () => {
        cy.get(entityEditButtonSelector).first().click();
        cy.getEntityCreateUpdateHeading('Client');
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="firstName"]`).type('Gerson');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Gerson');

      cy.get(`[data-cy="lastName"]`).type('Simonis');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Simonis');

      cy.get(`[data-cy="email"]`).type('Evan.Stehr41@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Evan.Stehr41@hotmail.com');

      cy.get(`[data-cy="address"]`).type('even');
      cy.get(`[data-cy="address"]`).should('have.value', 'even');

      cy.get(`[data-cy="phone"]`).type('283-589-3422 x3983');
      cy.get(`[data-cy="phone"]`).should('have.value', '283-589-3422 x3983');
